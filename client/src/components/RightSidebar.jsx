...
const RightSidebar = () => {
  ...
  // getall the images from messages and set them to state
  ...
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll  ${selectedUser ? 'max-md:hidden' : ""} `}>
      ...
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light m-auto '>
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {
            msgImages.map((url, index) => (
              <div className="cursor-pointer rounded" key={index} onClick={() => window.open(url)}>
                <img src={url} alt={`media_${index}`} className=' h-full rounded-md' />
              </div>
            ))}
        </div>
