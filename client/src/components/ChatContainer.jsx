...
const ChatContainer = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    await sendMessage({text: input.trim(),
      receiverId: selectedUser._id /// changed for error
    });
      setInput("");
    }
  

  // handle sedding an image
    ...
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      {/* header */}
        <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
            <img src={selectedUser.profilePic || assets.avatar_icon} alt="profilemartin" className='w-8 rounded-full' />
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        ...
        .map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} alt={`message_image_${index}`} className='max-w-[230px] border border-gray-700  rounded-lg overflow-hidden mb-8' />
            ) : (
